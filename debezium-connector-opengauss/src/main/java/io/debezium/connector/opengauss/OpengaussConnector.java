...
public class OpengaussConnector extends RelationalBaseSourceConnector {
    ...
    protected void validateConnection(Map<String, ConfigValue> configValues, Configuration config) {
        ...
        final OpengaussConnectorConfig opengaussConfig = new OpengaussConnectorConfig(config);
        final ConfigValue hostnameValue = configValues.get(RelationalDatabaseConnectorConfig.HOSTNAME.name());
        // Try to connect to the database ...
        try (OpengaussConnection connection = new OpengaussConnection(opengaussConfig.getJdbcConfig())) {
            try {
                ...
                if (!"logical".equals(walLevel)) {
                    final String errorMessage = "Opengauss" +
                            " server wal_level property must be \"logical\" but is: " + walLevel;
                ...
                if (!connection.queryAndMap(
                    ...
                    final String errorMessage = "Opengauss roles LOGIN and REPLICATION are not assigned to user: " + connection.username();
                    LOGGER.error(errorMessage);
                }
            }
    ...
    public List<TableId> getMatchingCollections(Configuration config) {
        OpengaussConnectorConfig connectorConfig = new OpengaussConnectorConfig(config);
        try (OpengaussConnection connection = new OpengaussConnection(connectorConfig.getJdbcConfig())) {
