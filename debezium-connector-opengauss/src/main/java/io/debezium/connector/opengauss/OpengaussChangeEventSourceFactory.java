...
public class OpengaussChangeEventSourceFactory implements ChangeEventSourceFactory<OpengaussPartition, OpengaussOffsetContext> {
    ...
    private final MainConnectionProvidingConnectionFactory<OpengaussConnection> connectionFactory;
    ...
    public OpengaussChangeEventSourceFactory(OpengaussConnectorConfig configuration, SnapshotterService snapshotterService,
                                             MainConnectionProvidingConnectionFactory<OpengaussConnection> connectionFactory,
                                             ErrorHandler errorHandler, OpengaussEventDispatcher<TableId> dispatcher, Clock clock, OpengaussSchema schema,
                                             OpengaussTaskContext taskContext, ReplicationConnection replicationConnection, SlotCreationResult slotCreatedInfo,
    ...
    @Override
    public SnapshotChangeEventSource<OpengaussPartition, OpengaussOffsetContext> getSnapshotChangeEventSource(SnapshotProgressListener<OpengaussPartition> snapshotProgressListener,
                                                                                                              NotificationService<OpengaussPartition, OpengaussOffsetContext> notificationService) {
    ...
    @Override
    public Optional<IncrementalSnapshotChangeEventSource<OpengaussPartition, ? extends DataCollectionId>> getIncrementalSnapshotChangeEventSource(
