 ...
 */
public class OpengaussStreamingChangeEventSource implements StreamingChangeEventSource<OpengaussPartition, OpengaussOffsetContext> {
    ...
    private static final Logger LOGGER = LoggerFactory.getLogger(OpengaussStreamingChangeEventSource.class);
    ...
    private OpengaussOffsetContext effectiveOffset;

    public OpengaussStreamingChangeEventSource(OpengaussConnectorConfig connectorConfig, SnapshotterService snapshotterService,
                                               OpengaussConnection connection, OpengaussEventDispatcher<TableId> dispatcher, ErrorHandler errorHandler, Clock clock,
                                               OpengaussSchema schema, OpengaussTaskContext taskContext, ReplicationConnection replicationConnection) {
    ...
    @Override
    public void init(OpengaussOffsetContext offsetContext) {
        this.effectiveOffset = offsetContext == null ? OpengaussOffsetContext.initialContext(connectorConfig, connection, clock) : offsetContext;
    ...
    public void execute(ChangeEventSourceContext context, OpengaussPartition partition, OpengaussOffsetContext offsetContext)
        ...
        try {
            ...
            if (hasStartLsnStoredInContext) {
                ...
                LOGGER.info("No previous LSN found, streaming from the latest xlogpos or flushed LSN...");
            ...
            stream.startKeepAlive(Threads.newSingleThreadExecutor(OpengaussConnector.class, connectorConfig.getLogicalName(), KEEP_ALIVE_THREAD_NAME));
    ...
    private void processMessages(ChangeEventSourceContext context, OpengaussPartition partition, OpengaussOffsetContext offsetContext, final ReplicationStream stream)
    ...
    private void processReplicationMessages(OpengaussPartition partition, OpengaussOffsetContext offsetContext, ReplicationStream stream, ReplicationMessage message)
        ...
        // DML event
            ...
            if (message.getOperation() != Operation.NOOP) {
                tableId = OpengaussSchema.parse(message.getTable());
            ...
            boolean dispatched = message.getOperation() != Operation.NOOP && dispatcher.dispatchDataChangeEvent(
                    partition,
                    tableId,
                    new OpengaussChangeRecordEmitter(
                            partition,
                            offsetContext,
                            clock,
                            connectorConfig,
    ...
    private void searchWalPosition(ChangeEventSourceContext context, OpengaussPartition partition, OpengaussOffsetContext offsetContext,
    ...
    private void commitMessage(OpengaussPartition partition, OpengaussOffsetContext offsetContext, final Lsn lsn) throws SQLException, InterruptedException {
        lastCompletelyProcessedLsn = lsn;
    ...
    public void commitOffset(Map<String, ?> partition, Map<String, ?> offset) {
        try {
            ...
            final Lsn commitLsn = Lsn.valueOf((Long) offset.get(OpengaussOffsetContext.LAST_COMMIT_LSN_KEY));
            final Lsn changeLsn = Lsn.valueOf((Long) offset.get(OpengaussOffsetContext.LAST_COMPLETELY_PROCESSED_LSN_KEY));
     ...
     */
    private boolean isInPreSnapshotCatchUpStreaming(OpengaussOffsetContext offsetContext) {
