...
import io.debezium.connector.opengauss.connection.ogproto.OgProtoMessageDecoder;
import io.debezium.connector.opengauss.connection.wal2json.NonStreamingWal2JsonMessageDecoder;
import io.debezium.connector.opengauss.connection.wal2json.StreamingWal2JsonMessageDecoder;
...
public class OpengaussConnectorConfig extends RelationalDatabaseConnectorConfig {
    ...
    */
    ...
    public static final Field TOASTED_VALUE_PLACEHOLDER = Field.create("toasted.value.placeholder")
            ...
            .withImportance(Importance.MEDIUM)
            .withValidation(OpengaussConnectorConfig::validateToastedValuePlaceholder)
            .withDescription("Specify the constant that will be provided by Debezium to indicate that " +
                    "the original value is a toasted value not provided by the database. " +
                    "If starts with 'hex:' prefix it is expected that the rest of the string represents hexadecimal encoded octets." +
    ...
    protected SourceInfoStructMaker<? extends AbstractSourceInfo> getSourceInfoStructMaker(Version version) {
        switch (version) {
            case V1:
                return new LegacyV1OpengaussSourceInfoStructMaker(Module.name(), Module.version(), this);
            default:
                return new OpengaussSourceInfoStructMaker(Module.name(), Module.version(), this);
        }
    }
    ...
    */
    ...
    public enum LogicalDecoder implements EnumeratedValue {
        ...
        DECODERBUFS("decoderbufs") {
            @Override
            public MessageDecoder messageDecoder(MessageDecoderContext config) {
                return new OgProtoMessageDecoder();
            }
