 ...
 */
package io.debezium.connector.opengauss.pipeline.txmetadata;
...
/**
 * Postgres specific overrides for {@link TransactionMonitor}.
 *
 * @author vjuranek
 */
public class OpengaussTransactionMonitor extends TransactionMonitor {

    public OpengaussTransactionMonitor(CommonConnectorConfig connectorConfig, EventMetadataProvider eventMetadataProvider,
    ...
    private Struct adjustTxId(Struct txStruct, OffsetContext offsetContext) {
        final String lsn = Long.toString(((OpengaussOffsetContext) offsetContext).asOffsetState().lastSeenLsn().asLong());
