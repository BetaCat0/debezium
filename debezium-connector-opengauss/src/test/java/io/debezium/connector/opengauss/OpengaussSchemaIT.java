...
/**
 * Unit test for {@link OpengaussSchema}
 *
 * @author Horia Chiorean (hchiorea@redhat.com)
 */
public class OpengaussSchemaIT {
    ...
    public void shouldLoadSchemaForExtensionPostgresTypes() throws Exception {
        TestHelper.executeDDL("opengauss_create_tables.ddl");
        OpengaussConnectorConfig config = new OpengaussConnectorConfig(
                TestHelper.defaultConfig().with(OpengaussConnectorConfig.INCLUDE_UNKNOWN_DATATYPES, true).build());
    ...
    public void shouldApplyFilters() throws Exception {
        ...
        OpengaussConnectorConfig config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(SCHEMA_EXCLUDE_LIST, "s1").build());
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(SCHEMA_BLACKLIST, "s1").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
            assertTablesIncluded("s2.a", "s2.b");
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(SCHEMA_EXCLUDE_LIST, "s.*").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(SCHEMA_BLACKLIST, "s.*").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.create()) {
            schema.refresh(connection, false);
            assertTablesExcluded("s1.a", "s2.a", "s1.b", "s2.b");
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.TABLE_EXCLUDE_LIST, "s1.A,s2.A").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.TABLE_BLACKLIST, "s1.A,s2.A").build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
            assertTablesIncluded("s1.b", "s2.b");
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig()
                .with(SCHEMA_EXCLUDE_LIST, "s2")
                .with(OpengaussConnectorConfig.TABLE_EXCLUDE_LIST, "s1.A")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.TABLE_BLACKLIST, "s1.A")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.COLUMN_EXCLUDE_LIST, ".*aa")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.COLUMN_BLACKLIST, ".*aa")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.COLUMN_INCLUDE_LIST, ".*bb")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
        ...
        config = new OpengaussConnectorConfig(TestHelper.defaultConfig().with(OpengaussConnectorConfig.COLUMN_WHITELIST, ".*bb")
                .build());
        schema = TestHelper.getSchema(config, typeRegistry);
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            schema.refresh(connection, false);
    ...
    public void shouldProperlyGetDefaultColumnValues() throws Exception {
        ...
        final OpengaussConnection.PostgresValueConverterBuilder valueConverterBuilder = (typeRegistry) -> OpengaussValueConverter.of(
                config,
        ...
        try (OpengaussConnection connection = TestHelper.createWithTypeRegistry()) {
            ...
            OpengaussDefaultValueConverter defaultValueConverter = connection.getDefaultValueConverter();
    ...
    private void assertColumnDefault(String columnName, Object expectedDefault, List<Column> columns, OpengaussDefaultValueConverter defaultValueConverter) {
