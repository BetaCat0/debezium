...
/**
 * Integration test for {@link io.debezium.transforms.outbox.EventRouter} with {@link OpengaussConnector}
 *
 * @author Renato Mefi (gh@mefi.in)
 */
public class OutboxEventRouterIT extends AbstractEventRouterTest<OpengaussConnector> {
    ...
    @Override
    protected Class<OpengaussConnector> getConnectorClass() {
    ...
    protected Configuration.Builder getConfigurationBuilder(boolean initialSnapshot) {
        OpengaussConnectorConfig.SnapshotMode snapshotMode = initialSnapshot ? OpengaussConnectorConfig.SnapshotMode.INITIAL
                : OpengaussConnectorConfig.SnapshotMode.NO_DATA;
        return TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, snapshotMode.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "outboxsmtit")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "outboxsmtit\\.outbox");
    }
