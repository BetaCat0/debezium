...
import io.debezium.connector.opengauss.connection.OpengaussReplicationConnection;
...
import io.debezium.connector.opengauss.junit.SkipWhenDecoderPluginNameIs;
import io.debezium.connector.opengauss.junit.SkipWhenDecoderPluginNameIsNot;
...
import static io.debezium.connector.opengauss.TestHelper.PK_FIELD;
import static io.debezium.connector.opengauss.TestHelper.topicName;
...
/**
 * Integration test for {@link OpengaussConnector} using an {@link io.debezium.embedded.EmbeddedEngine}
 *
 * @author Horia Chiorean (hchiorea@redhat.com)
 */
public class OpengaussConnectorIT extends AbstractConnectorTest {
    ...
    private OpengaussConnector connector;
    ...
    public void shouldValidateConnectorConfigDef() {
        connector = new OpengaussConnector();
        ConfigDef configDef = connector.config();
        assertThat(configDef).isNotNull();
        OpengaussConnectorConfig.ALL_FIELDS.forEach(this::validateFieldDef);
    }
    ...
    public void shouldNotStartWithInvalidConfiguration() throws Exception {
        ...
        start(OpengaussConnector.class, config, (success, msg, error) -> {
    ...
    public void shouldValidateMinimalConfiguration() throws Exception {
        ...
        Config validateConfig = new OpengaussConnector().validate(config.asMap());
    ...
    public void shouldNotStartWithInvalidSlotConfigAndUserRoles() throws Exception {
        ...
        TestHelper.dropPublication();
        TestHelper.dropDefaultReplicationSlot();
        TestHelper.executeDDL("opengauss_create_tables.ddl");
        TestHelper.execute("DROP USER IF EXISTS badboy CASCADE;");
        TestHelper.execute("CREATE USER badboy WITH PASSWORD 'badboy@1234';", "GRANT ALL PRIVILEGES ON DATABASE postgres TO badboy;");

        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.NO_DATA)
                .with(OpengaussConnectorConfig.SLOT_NAME, ReplicationConnection.Builder.DEFAULT_SLOT_NAME)
                .build();

        start(OpengaussConnector.class, config);
        waitForStreamingRunning();

        Configuration failingConfig = TestHelper.defaultConfig()
                .with("name", "failingPGConnector")
                .with(OpengaussConnectorConfig.DATABASE_CONFIG_PREFIX + JdbcConfiguration.USER, "badboy")
                .with(OpengaussConnectorConfig.DATABASE_CONFIG_PREFIX + JdbcConfiguration.PASSWORD, "badboy@1234")
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.NO_DATA)
                .with(OpengaussConnectorConfig.SLOT_NAME, ReplicationConnection.Builder.DEFAULT_SLOT_NAME)
                .build();
        List<ConfigValue> validatedConfig = new OpengaussConnector().validate(failingConfig.asMap()).configValues();
    ...
    public void shouldValidateConfiguration() throws Exception {
        // use an empty configuration which should be invalid because of the lack of DB connection details
        Configuration config = Configuration.create().build();
        OpengaussConnector connector = new OpengaussConnector();
        Config validatedConfig = connector.validate(config.asMap());
        // validate that the required fields have errors
        assertConfigurationErrors(validatedConfig, OpengaussConnectorConfig.HOSTNAME, 1);
        assertConfigurationErrors(validatedConfig, OpengaussConnectorConfig.USER, 1);
        assertConfigurationErrors(validatedConfig, OpengaussConnectorConfig.DATABASE_NAME, 1);
        assertConfigurationErrors(validatedConfig, CommonConnectorConfig.TOPIC_PREFIX, 1);

        // validate the non required fields
        validateConfigField(validatedConfig, OpengaussConnectorConfig.PLUGIN_NAME, OpengaussConnectorConfig.LogicalDecoder.PGOUTPUT.getValue());
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SLOT_NAME, ReplicationConnection.Builder.DEFAULT_SLOT_NAME);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.PORT, OpengaussConnectorConfig.DEFAULT_PORT);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.MAX_QUEUE_SIZE, OpengaussConnectorConfig.DEFAULT_MAX_QUEUE_SIZE);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.MAX_BATCH_SIZE, OpengaussConnectorConfig.DEFAULT_MAX_BATCH_SIZE);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SNAPSHOT_FETCH_SIZE, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.POLL_INTERVAL_MS, OpengaussConnectorConfig.DEFAULT_POLL_INTERVAL_MILLIS);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_MODE, OpengaussConnectorConfig.SecureConnectionMode.DISABLED);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_CLIENT_CERT, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_CLIENT_KEY, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_CLIENT_KEY_PASSWORD, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_ROOT_CERT, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SCHEMA_EXCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.TABLE_INCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.TABLE_EXCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.COLUMN_EXCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.COLUMN_INCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.MSG_KEY_COLUMNS, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL);
        ...
        validateConfigField(validatedConfig, OpengaussConnectorConfig.TIME_PRECISION_MODE, TemporalPrecisionMode.ADAPTIVE);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.DECIMAL_HANDLING_MODE, OpengaussConnectorConfig.DecimalHandlingMode.PRECISE);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.SSL_SOCKET_FACTORY, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.TCP_KEEPALIVE, true);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.LOGICAL_DECODING_MESSAGE_PREFIX_EXCLUDE_LIST, null);
        validateConfigField(validatedConfig, OpengaussConnectorConfig.LOGICAL_DECODING_MESSAGE_PREFIX_INCLUDE_LIST, null);
    }

    @Test
    public void shouldValidateReplicationSlotName() throws Exception {
        Configuration config = Configuration.create()
                .with(OpengaussConnectorConfig.SLOT_NAME, "xx-aa")
                .build();
        OpengaussConnector connector = new OpengaussConnector();
        Config validatedConfig = connector.validate(config.asMap());

        assertConfigurationErrors(validatedConfig, OpengaussConnectorConfig.SLOT_NAME, 1);
    }
    ...
    public void shouldSupportSSLParameters() throws Exception {
        ...
        // SSL is enabled
        Configuration config = TestHelper.defaultConfig().with(OpengaussConnectorConfig.SSL_MODE,
                OpengaussConnectorConfig.SecureConnectionMode.REQUIRED).build();
        start(OpengaussConnector.class, config, (success, msg, error) -> {
    ...
    public void shouldProduceEventsWithInitialSnapshot() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        start(OpengaussConnector.class, configBuilder.build());
        assertConnectorIsRunning();
        ...
        start(OpengaussConnector.class, configBuilder.with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE).build());
        assertConnectorIsRunning();
    ...
    public void shouldUseMillisecondsForTransactionCommitTime() throws InterruptedException {
        ...
        start(OpengaussConnector.class, TestHelper.defaultConfig().build());
        assertConnectorIsRunning();
    ...
    public void shouldConsumeMessagesFromSnapshot() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.MAX_QUEUE_SIZE, recordCount / 2)
                .with(OpengaussConnectorConfig.MAX_BATCH_SIZE, 10)
    ...
    public void shouldReceiveChangesForChangePKColumnDefinition() throws Exception {
        Testing.Print.enable();
        final String slotName = "pkcolumndef";
        TestHelper.create().dropReplicationSlot(slotName);
        try {
            final OpengaussConnectorConfig config = new OpengaussConnectorConfig(TestHelper.defaultConfig()
                    .with(OpengaussConnectorConfig.INCLUDE_UNKNOWN_DATATYPES, Boolean.FALSE)
                    .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "changepk")
                    .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                    .with(OpengaussConnectorConfig.SLOT_NAME, slotName)
                    .build());
            ...
            TestHelper.execute("INSERT INTO changepk.test_table(text) VALUES ('insert');");

            start(OpengaussConnector.class, config.getConfig());
            ...
            TestHelper.execute("INSERT INTO changepk.test_table VALUES(2, 'newpkcol')");
            records = consumeRecordsByTopic(1);

            SourceRecord insertRecord = records.recordsForTopic(topicName).get(0);
            assertEquals(topicName, insertRecord.topic());
            VerifyRecord.isValidInsert(insertRecord, newPkField, 2);

            // use INT4 instead of SERIAL, opengauss does not support SERIAL in column alteration(but it works when creating a new table)
            TestHelper.execute(
                    "ALTER TABLE changepk.test_table ADD COLUMN pk2 INT4 DEFAULT 0;"
                            + "ALTER TABLE changepk.test_table DROP CONSTRAINT test_table_pkey;"
                            + "ALTER TABLE changepk.test_table ADD PRIMARY KEY(newpk,pk2);");
            TestHelper.execute("INSERT INTO changepk.test_table VALUES(3, 'newpkcol', 8)");
            records = consumeRecordsByTopic(1);
            ...
            TestHelper.execute(
                    "ALTER TABLE changepk.test_table DROP CONSTRAINT test_table_pkey;"
                            ...
                            + "ALTER TABLE changepk.test_table ADD COLUMN pk3 INT4 DEFAULT 0;"
                            + "ALTER TABLE changepk.test_table ADD PRIMARY KEY(newpk,pk3);");
            TestHelper.execute("INSERT INTO changepk.test_table VALUES(5, 'dropandaddpkcol',10)");
    ...
    public void shouldReceiveChangesForChangeColumnDefault() throws Exception {
        Testing.Print.enable();
        final String slotName = "default_change";
        TestHelper.create().dropReplicationSlot(slotName);
        try {
            final OpengaussConnectorConfig config = new OpengaussConnectorConfig(TestHelper.defaultConfig()
                    .with(OpengaussConnectorConfig.INCLUDE_UNKNOWN_DATATYPES, Boolean.FALSE)
                    .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "default_change")
                    .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                    .with(OpengaussConnectorConfig.SLOT_NAME, slotName)
                    .build());
            ...
            TestHelper.execute("INSERT INTO default_change.test_table(i, text, tstz) VALUES (DEFAULT, DEFAULT, DEFAULT);");
    ...
    public void showThatSchemaColumnDefaultMayApplyRetroactively() throws Exception {
        ...
        try {
            ...
            TestHelper.execute(
                    ...
                    "ALTER TABLE default_change.test_table ADD COLUMN tstz TIMESTAMPTZ DEFAULT '2021-03-20 14:44:28 +1'::TIMESTAMPTZ;");
            TestHelper.execute("INSERT INTO default_change.test_table(i, text, bi, tstz) VALUES (DEFAULT, DEFAULT, DEFAULT, DEFAULT);");
    ...
    public void shouldIgnoreViews() throws Exception {
        TestHelper.execute(CREATE_TABLES_STMT);
        TestHelper.execute(INSERT_STMT);
        TestHelper.execute("CREATE VIEW s1.myview AS SELECT * from s1.a;");
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        start(OpengaussConnector.class, configBuilder.build());
    ...
    public void shouldExecuteOnConnectStatements() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.ON_CONNECT_STATEMENTS, "INSERT INTO s1.a (aa) VALUES (2); INSERT INTO s2.a (aa, bb) VALUES (2, 'hello;; world');")
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        start(OpengaussConnector.class, configBuilder.build());
        assertConnectorIsRunning();
        waitForStreamingRunning();
    ...
    public void shouldProduceEventsWhenSnapshotsAreNeverAllowed() throws InterruptedException {
        ...
        Configuration config = TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
    ...
    public void shouldNotProduceEventsWithInitialOnlySnapshot() throws InterruptedException {
        ...
        TestHelper.execute(INSERT_STMT);
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL_ONLY.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
    ...
    public void shouldProduceEventsWhenAlwaysTakingSnapshots() throws InterruptedException {
        ...
        TestHelper.execute(INSERT_STMT);
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.ALWAYS.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE);
        start(OpengaussConnector.class, configBuilder.build());
        assertConnectorIsRunning();
        waitForSnapshotToBeCompleted();
    ...
    public void shouldResumeSnapshotIfFailingMidstream() throws Exception {
        ...
        start(OpengaussConnector.class, configBuilder.build(), completionCallback, stopOnPKPredicate(2));
    ...
    public void shouldRecoverFromRetriableException() throws Exception {
        ...
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
        waitForStreamingRunning("opengauss", TestHelper.TEST_SERVER);
    ...
    public void shouldTakeExcludeListFiltersIntoAccount() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_EXCLUDE_LIST, "s2")
                .with(OpengaussConnectorConfig.TABLE_EXCLUDE_LIST, ".+b")
                .with(OpengaussConnectorConfig.COLUMN_EXCLUDE_LIST, ".+bb");
    ...
    public void shouldTakeBlacklistFiltersIntoAccount() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s2")
                .with(OpengaussConnectorConfig.TABLE_EXCLUDE_LIST, ".+b")
                .with(OpengaussConnectorConfig.COLUMN_EXCLUDE_LIST, ".+bb");
    ...
    public void shouldTakeColumnIncludeListFilterIntoAccount() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.COLUMN_INCLUDE_LIST, ".+aa,.+cc");
    ...
    public void shouldRemoveWhiteSpaceChars() throws Exception {
        TestHelper.execute(CREATE_TABLES_STMT);
        TestHelper.execute(INSERT_STMT);
        TestHelper.execute("CREATE TABLE s1.b (pk SERIAL, aa integer, PRIMARY KEY(pk));");
        TestHelper.execute("INSERT INTO s1.b (aa) VALUES (123);");
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s1")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, tableWhitelistWithWhitespace);
    ...
    public void shouldCloseTxAfterTypeQuery() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s1")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "s1.b")
                .with(OpengaussConnectorConfig.INCLUDE_UNKNOWN_DATATYPES, true);
    ...
    public void shouldReplaceInvalidTopicNameCharacters() throws Exception {
        ...
        TestHelper.execute("CREATE TABLE s1.\"dbz_878_some|test@data\" (pk SERIAL, aa integer, PRIMARY KEY(pk));");
        TestHelper.execute("INSERT INTO s1.\"dbz_878_some|test@data\" (aa) VALUES (123);");

        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s1")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "s1\\.dbz_878_some\\|test@data");
    ...
    public void shouldNotSendEmptyOffset() throws InterruptedException, SQLException {
        ...
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.NO_DATA.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "s1.a")
    ...
    public void shouldRegularlyFlushLsn() throws InterruptedException, SQLException {
        ...
        try (final OpengaussConnection connection = TestHelper.create()) {
    ...
    public void shouldRegularlyFlushLsnWithTxMonitoring() throws InterruptedException, SQLException {
        ...
        Configuration config = TestHelper.defaultConfig()
                ...
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "s1.a")
                .with(OpengaussConnectorConfig.PROVIDE_TRANSACTION_METADATA, true)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
        waitForStreamingRunning("opengauss", TestHelper.TEST_SERVER);
    ...
    public void shouldAllowForCustomSnapshot() throws InterruptedException {
        ...
        TestHelper.execute(INSERT_STMT);
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.CUSTOM.getValue())
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE_CUSTOM_NAME, CustomTestSnapshot.class.getName())
                .with(OpengaussConnectorConfig.SNAPSHOT_QUERY_MODE, CommonConnectorConfig.SnapshotQueryMode.CUSTOM)
                .with(OpengaussConnectorConfig.SNAPSHOT_QUERY_MODE_CUSTOM_NAME, CustomTestSnapshot.class.getName())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
    ...
    public void shouldAllowForSelectiveSnapshot() throws InterruptedException {
        ...
        TestHelper.execute(INSERT_STMT);
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.ALWAYS.name())
        ...
        start(OpengaussConnector.class, configBuilder
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE_TABLES, "s2.a")
                .build());
        assertConnectorIsRunning();
    ...
    public void exportedSnapshotShouldNotSkipRecordOfParallelTx() throws Exception {
        ...
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                .with(OpengaussConnectorConfig.MAX_QUEUE_SIZE, 2)
                .with(OpengaussConnectorConfig.MAX_BATCH_SIZE, 1)
                .build();
        final OpengaussConnection pgConnection = TestHelper.create();
        pgConnection.setAutoCommit(false);
        pgConnection.executeWithoutCommitting(INSERT_STMT);
        final AtomicBoolean inserted = new AtomicBoolean();
        start(OpengaussConnector.class, config, loggingCompletion(), x -> false, x -> {
            if (!inserted.get()) {
                TestHelper.execute(INSERT_STMT);
                try {
                    pgConnection.commit();
    ...
    @Test
    @FixFor("DBZ-2288")
    public void exportedSnapshotShouldNotSkipRecordOfParallelTxMppdbDecoding() throws Exception {
        TestHelper.dropDefaultReplicationSlot();
        TestHelper.createDefaultReplicationSlot();
    ...
    public void shouldResumeStreamingFromSlotPositionForCustomSnapshot() throws Exception {
        ...
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.CUSTOM.getValue())
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE_CUSTOM_NAME, CustomStartFromStreamingTestSnapshot.class.getName())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
    ...
    public void customSnapshotterSkipsTablesOnRestart() throws Exception {
        ...
        config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.CUSTOM.getValue())
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE_CUSTOM_NAME, CustomPartialTableTestSnapshot.class.getName())
                .with(OpengaussConnectorConfig.SNAPSHOT_QUERY_MODE, CommonConnectorConfig.SnapshotQueryMode.CUSTOM)
                .with(OpengaussConnectorConfig.SNAPSHOT_QUERY_MODE_CUSTOM_NAME, CustomPartialTableTestSnapshot.class.getName())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.FALSE)
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
        ...
        Awaitility.await()
                ...
                .until(() -> {
                    ...
                    return (boolean) ManagementFactory.getPlatformMBeanServer()
                            .getAttribute(getSnapshotMetricsObjectName("opengauss", TestHelper.TEST_SERVER), "SnapshotCompleted");
                });
    ...
    public void testCustomSnapshotterSnapshotCompleteLifecycleHook() throws Exception {
        ...
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.CUSTOM.getValue())
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE_CUSTOM_NAME, CustomLifecycleHookTestSnapshot.class.getName())
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
    ...
    private String getConfirmedFlushLsn(OpengaussConnection connection) throws SQLException {
        final String lsn = connection.prepareQueryAndMap(
                "select * from pg_replication_slots where slot_name = ? and database = ? and plugin = ?", statement -> {
                    ...
                    statement.setString(3, TestHelper.decoderPlugin().getOpengaussPluginName());
                },
                rs -> {
                    if (rs.next()) {
                        return rs.getString("confirmed_flush");
                    } else {
    ...
    public void testEmptySchemaWarningAfterApplyingFilters() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL_ONLY.getValue())
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "my_products");
    ...
    public void testCustomPublicationNameUsed() throws Exception {
        // This captures all logged messages, allowing us to verify log message was written.
        final LogInterceptor logInterceptor = new LogInterceptor(OpengaussReplicationConnection.class);
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.PUBLICATION_NAME, "cdc");

        start(OpengaussConnector.class, configBuilder.build());
        assertConnectorIsRunning();
        waitForAvailableRecords(100, TimeUnit.MILLISECONDS);

        // publication will be dropped after connector is stopped
    ...
    public void shouldRewriteIdentityKey() throws InterruptedException {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.INITIAL.getValue())
                .with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s1,s2")
                // rewrite key from table 'a': from {pk} to {pk, aa}
                .with(OpengaussConnectorConfig.MSG_KEY_COLUMNS, "(.*)1.a:pk,aa");
    ...
    public void shouldNotIssueWarningForNoMonitoredTablesAfterApplyingFilters() throws Exception {
        ...
        TestHelper.execute(INSERT_STMT);
        TestHelper.execute(INSERT_STMT);
        Configuration config = TestHelper.defaultConfig().with(OpengaussConnectorConfig.SCHEMA_INCLUDE_LIST, "s2").build();
    ...
    public void shouldCreatePublicationWhenReplicationSlotExists() throws Exception {
        ...
        final LogInterceptor interceptor = new LogInterceptor(OpengaussReplicationConnection.class);
        start(OpengaussConnector.class, config);
        waitForStreamingRunning();
    ...
    public void shouldConsumeEventsWithMaskedHashedColumns() throws Exception {
        TestHelper.execute(CREATE_TABLES_STMT);
        TestHelper.execute(INSERT_STMT);
        TestHelper.execute("CREATE TABLE s2.b (pk SERIAL, bb varchar(255), PRIMARY KEY(pk));");
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
    ...
    public void shouldConfigureSubscriptionsFromTableFilters() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.PUBLICATION_NAME, "cdc")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "public.numeric_table,public.text_table,s1.a,s2.a")
                .with(OpengaussConnectorConfig.PUBLICATION_AUTOCREATE_MODE, OpengaussConnectorConfig.AutoCreateMode.FILTERED.getValue());
    ...
    public void shouldThrowWhenAutocreationIsDisabled() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SLOT_NAME, "cdc")
                .with(OpengaussConnectorConfig.PUBLICATION_AUTOCREATE_MODE, OpengaussConnectorConfig.AutoCreateMode.DISABLED.getValue());
        ...
        start(OpengaussConnector.class, configBuilder.build(), cb);
        waitForAvailableRecords(100, TimeUnit.MILLISECONDS);
    ...
    public void shouldProduceMessagesOnlyForConfiguredTables() throws Exception {
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.PUBLICATION_NAME, "cdc")
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "s2.a")
                .with(OpengaussConnectorConfig.PUBLICATION_AUTOCREATE_MODE, OpengaussConnectorConfig.AutoCreateMode.FILTERED.getValue());
    ...
    public void shouldThrowWhenTableFiltersIsEmpty() throws Exception {
        final LogInterceptor logInterceptor = new LogInterceptor(OpengaussConnectorIT.class);
        ...
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.PUBLICATION_NAME, "cdc")
                .with(OpengaussConnectorConfig.PUBLICATION_AUTOCREATE_MODE, OpengaussConnectorConfig.AutoCreateMode.FILTERED.getValue())
                .with(OpengaussConnectorConfig.TABLE_INCLUDE_LIST, "nonexistent.table");
    ...
    public void shouldEmitNoEventsForSkippedCreateOperations() throws Exception {
        ...
        Configuration config = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.NO_DATA.getValue())
                .with(OpengaussConnectorConfig.DROP_SLOT_ON_STOP, Boolean.TRUE)
                .with(OpengaussConnectorConfig.SKIPPED_OPERATIONS, Envelope.Operation.UPDATE.code())
                .build();
        start(OpengaussConnector.class, config);
        assertConnectorIsRunning();
        waitForStreamingRunning("opengauss", TestHelper.TEST_SERVER);
    ...
    public void shouldHaveLastCommitLsn() throws InterruptedException {
        TestHelper.execute(CREATE_TABLES_STMT);
        TestHelper.execute(INSERT_STMT);
        start(OpengaussConnector.class, TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.PROVIDE_TRANSACTION_METADATA, true)
                .with(OpengaussConnectorConfig.SNAPSHOT_MODE, OpengaussConnectorConfig.SnapshotMode.NO_DATA.getValue())
                .build());
        assertConnectorIsRunning();
    ...
    public void testCreateNumericReplicationSlotName() throws Exception {
        ...
        TestHelper.execute(INSERT_STMT);
        Configuration.Builder configBuilder = TestHelper.defaultConfig()
                .with(OpengaussConnectorConfig.SLOT_NAME, "12345");
        start(OpengaussConnector.class, configBuilder.build());
        waitForStreamingRunning();
        assertConnectorIsRunning();
    }
    ...
    private void waitForSnapshotToBeCompleted() throws InterruptedException {
        waitForSnapshotToBeCompleted("opengauss", TestHelper.TEST_SERVER);
    }
